...
function nameOf(item) { return item.name || item.categoryName || ""; }
...
async function edit(item) {
  Object.assign(form, { id: item.id, name: nameOf(item), type: item.type || "expense" });
  formOpen.value = true;
  await nextTick();
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="item in categories" :key="item.id">
              <td>{{ item.id }}</td><td>{{ nameOf(item) }}</td><td>{{ typeText(item.type) }}</td>
