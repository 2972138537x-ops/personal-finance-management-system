...
| 前端：Vue 3, Vite, HTML, CSS, JavaScript | フロントエンド：Vue 3, Vite, HTML, CSS, JavaScript |
...
| 收支分类管理 | 収支カテゴリ管理 |
| 新用户默认分类初始化 | 新規ユーザーのデフォルトカテゴリ自動作成 |
| 收支记录管理 | 収支記録管理 |
...
| 默认分类根据语言自动显示 | デフォルトカテゴリ名の多言語表示 |
| 响应式布局，支持 PC 和手机端 | レスポンシブレイアウト、PC・スマートフォン対応 |
| 深色模式和主题色设置 | ダークモードとテーマカラー設定 |
| 密码显示 / 隐藏 | パスワード表示 / 非表示 |
| 金额输入校验 | 金額入力チェック |
...
| 中文 | 日本語 |
|---|---|
| 新用户注册后自动生成常用收入 / 支出默认分类 | 新規登録後、一般的な収入 / 支出カテゴリを自動作成 |
...
| 默认分类使用 `code` 和 `is_default` 标识，前端根据语言显示名称 | デフォルトカテゴリは `code` と `is_default` で管理し、画面では言語に応じて表示 |
| 用户手动编辑默认分类后，分类会转为自定义分类 | ユーザーがデフォルトカテゴリを編集した場合、カスタムカテゴリとして扱う |
...
| 中文 | 日本語 |
|---|---|
| 使用 Vue 3 + Vite 实现组件化前端 | Vue 3 + Vite でコンポーネント化されたフロントを実装 |
...
| PC 和手机端布局适配 | PC とスマートフォン表示に対応 |
| 统计图、移动端卡片、管理员页面移动端适配 | グラフ、モバイルカード、管理者画面のモバイル対応 |
...
### 6. 打包前端到 Spring Boot / フロントを Spring Boot にビルド
...
| 中文 | 日本語 | URL |
|---|---|---|
| 前端页面（Spring Boot 正式访问） | フロント画面（Spring Boot からアクセス） | http://localhost:8080/ |
| 前端页面（Vite 开发模式） | フロント画面（Vite 開発モード） | http://localhost:5173/ |
...
| Vue 3 + Vite 实现组件化前端，支持 PC 和手机端 | Vue 3 + Vite でコンポーネント化し、PC・スマートフォンに対応 |
| 前端支持中日英切换和密码显示 / 隐藏 | フロントは中日英切り替えとパスワード表示 / 非表示に対応 |
| 新用户注册后自动初始化默认分类 | 新規登録後にデフォルトカテゴリを自動作成 |
| 默认分类根据语言显示，自定义分类保持用户输入名称 | デフォルトカテゴリは言語に応じて表示し、カスタムカテゴリはユーザー入力名を保持 |
